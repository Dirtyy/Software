...
depTime = input('Enter the Departure Time: ')

def formatChecker(str1, str2):
  if len(arrTime) < 5:
    return '0' + arrTime
  elif len(depTime) < 5:
    return '0' + depTime

formatChecker(arrTime, depTime)

def totalTime(arr, dep):
        time1 = datetime.strptime(arr, '%H:%M')
        time2 = datetime.strptime(dep, '%H:%M') #Taking in the inputs an assigning them to a datetime object
